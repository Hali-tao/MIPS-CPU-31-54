...
module cpu(
    ...
    wire is_md_inst = (hi_write | lo_write) & ~whilo_src;
    wire md_busy;
    reg md_started;
    always @(posedge clk or posedge rst) begin
        if(rst) begin
            ...
            if(is_md_inst && !md_started) begin
                md_started <= 1'b1;
            end else if(md_started && !md_busy) begin
                md_started <= 1'b0;
            end
        end
    end

    wire md_start = is_md_inst & !md_started;
    // wire stall = is_md_inst & (md_start | (md_busy && md_started));
    wire stall = is_md_inst & md_busy;
    ...
    wire actual_hi_we = whilo_src ? (hi_write & ~exception) : (hi_write & ~md_busy & md_started & ~exception);
    wire actual_lo_we = whilo_src ? (lo_write & ~exception) : (lo_write & ~md_busy & md_started & ~exception);

    // ==================== 3. 基础 Datapath (PC / EXT / RegFile) ====================
    pc u_pc(.clk(clk), .rst(rst), .stall(stall), .npc(pc_next), .pc(pc_out));
    ...
    hilo_reg u_hilo(
        ...
        .hi_we(actual_hi_we),
        .lo_we(actual_lo_we),
        .hi_in(whilo_src ? rdata1 : md_hi), 
        .lo_in(whilo_src ? rdata1 : md_lo),
        .hi_out(hi_out), 
        .lo_out(lo_out)
    );
